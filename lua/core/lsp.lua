...
-- Import the statusline modules
local statusline_modules = require("core.statusline.init")
...
local components = {
    git_branch = function()
        return statusline_modules.git_branch()
    end,
    
    harpoon = function()
        return statusline_modules.harpoon()
    end,
    
    file = function()
        return "%f%m%r"  -- filename, modified, readonly
    end,
    
    python_venv = function()
        return statusline_modules.python_venv()
    end,
    
    clients = function()
        return statusline_modules.clients()
    end,
    ...
    total_lines = function()
        return statusline_modules.total_lines()
    end,
...
-- Build statusline from order
local function build_statusline_string()
    local parts = {}
    
    -- Add left components
    table.insert(parts, " ")
    for _, component_name in ipairs(statusline_order.left) do
        if components[component_name] then
            local result = components[component_name]()
            -- Only add if it's not a vim pattern (like %f)
            if type(result) == "string" and result:sub(1, 1) == "%" then
                table.insert(parts, result)
            else
                table.insert(parts, "%{%v:lua.statusline_component('" .. component_name .. "')%}")
            end
            table.insert(parts, " ")
        end
    end
    ...
    for _, component_name in ipairs(statusline_order.right) do
        if components[component_name] then
            local result = components[component_name]()
            if type(result) == "string" and result:sub(1, 1) == "%" then
                table.insert(parts, result)
...
-- Export component accessor to global scope
function _G.statusline_component(name)
...
vim.opt.statusline = table.concat({
    " ",
    "%{%v:lua.statusline_component('git_branch')%}",
    " ",
    "%{%v:lua.statusline_component('harpoon')%}",
    " ",
    "%f%m%r",
    "%{%v:lua.statusline_component('python_venv')%}",
    "%=",
    "%{%v:lua.statusline_component('clients')%}",
    " %l:%c ",
    "%{%v:lua.statusline_component('total_lines')%}",
    " %p%% ",
}, "")

-- Refresh statusline on events
vim.api.nvim_create_autocmd(
    { "BufEnter", "BufWritePost", "LspAttach", "LspDetach", "DiagnosticChanged" },
    {
        callback = function()
